...
survlocspdf<-SpatialPointsDataFrame(
  coords=sp, 
  data=surveyloc #add data to make this a spatial points data frame
)
```
...
tm_shape(elevation)+ #loads file (either shape file, raster file, etc...)
  ...
  tm_shape(survlocspdf,
           is.master = T)+
 tm_symbols(size="Total",
              col="Sex",
            size.lim = c(1,300),
              title.size="Group Size",
              title.col = "Group Composition",
              title.shape = "Aerial Survey Locations")+
...
tm_shape(locspdf)+
  ...
  tm_layout(
    main.title = "Capture Locations and Winter Aerial Survey Locations of Elk",#text for main title
